...
#define I2C_BUS				   4
#define INTERFACE_TYPE			   0 // 0: I2S, 1: TDM
...
static const u8 IB3_FORMAT[5][4] = {
	/* 1x amp, 4 channels */
	{ IB3_FORMAT_TDM4 },
	/* 2x amp, 8 channels */
...
static int tda7802_digital_mute(struct snd_soc_dai *dai, int mute, int direction)
{
	struct device *dev = dai->dev;
	int err;
		...
		unsigned int rx_mask, int slots, int slot_width)
{
	...
	if (!(slots == 1 || slots == 4 || slots == 8 || slots == 16)) {
		dev_err(dev, "Failed to set %d slots, supported: 1, 4, 8, 16\n",
				slots);
		return -ENOTSUPP;
	}
...
static const struct snd_soc_dai_ops tda7802_dai_ops = {
	...
	.mute_stream = tda7802_digital_mute,
...
static struct snd_soc_dai_driver tda7802_stereo_dai = {
	.name = "tda7802-stereo",
	.playback = {
		.stream_name  = "Playback",
		.channels_min = 2,
